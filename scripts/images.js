...
const handleIos = (filePath, fileName, fileExtension, dataDir) => {
  ...
  if (!fs.existsSync(dataDir)) {
    ...
    fs.writeFile(`${dataDir}/Contents.json`, jsonContentIos, 'utf8', async () => {
...
const createAndroidVersion = async (
  ...
  // 3X
  await image.toFile(`${dataDir}/${androidPathX3}/${fileName}.${fileExtension}`);
...
const imageGenerator = async (img, dir = '') => {
  ...
  // iOS config
  const iosPath = './../ios/boilerplate/Images.xcassets/';
  const imgFolderExtension = '.imageset';

  const iosDataDir = path.resolve(`${__dirname}${iosPath}${fileName}${imgFolderExtension}`);
...
(() => {
  ...
  if (argv.indexOf('.') > -1) {
    ...
    console.log(chalk.red('Folders and multiple images are not supported yet!'));
  }
})();
